...
// Revised timeline + animation + NOAA WMS playback
// Append this to the end of your existing Cesium setup script

// Set start and end time for animation loop
const start = Cesium.JulianDate.fromDate(new Date("2025-01-01T00:00:00Z"));
const end = Cesium.JulianDate.fromDate(new Date("2025-01-10T00:00:00Z"));
...
viewer.clock.multiplier = 86400; // 1 day per tick
viewer.timeline.zoomTo(start, end);

// Modify switchLayers to accept dynamic time
function switchLayers(metric, noaaMetric) {
  ...
  if (metric !== "none") {
    activeLayer = viewer.imageryLayers.addImageryProvider(
      createWMSTileLayer(metric, formattedDate)
    );
...
// Auto update layer on each tick
viewer.clock.onTick.addEventListener(() => {
  const selectedMetric = document.querySelector("button.active")?.textContent.toLowerCase() || "none";
  const selectedNoaaMetric = metricToNoaa[selectedMetric] || "none";
...
datePicker.value = new Date(
  new Date(staticDateEST.toISOString()).getTime() - 2 * 24 * 60 * 60 * 1000 // staticDate - 2 day
)
  .toISOString()
  .split("T")[0];
...
// Auto-load DHW layer on startup
(() => {
  const selectedMetric = "dhw";
  const selectedNoaaMetric = metricToNoaa[selectedMetric];
  const selectedDate = datePicker.value;
  const [year, month, day] = selectedDate.split("-");
  const noaaTime = `${selectedDate}T12:00:00.000Z`;
  ...
  // Highlight the DHW button visually
  document.querySelectorAll("button").forEach((btn) => {
    if (btn.textContent.toLowerCase() === "dhw") btn.classList.add("active");
  });
})();